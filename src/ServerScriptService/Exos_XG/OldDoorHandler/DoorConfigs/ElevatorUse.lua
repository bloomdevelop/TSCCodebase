...
return function(p,btn)
	...
	if not elevatorMoving then
		...
		if Down.Value == false then  
			...
			local Tween = game:GetService("TweenService"):Create(carriage.Union11,info,{Position = UpPosition.Value})
			Tween:Play()
			--animator.ELEVATEUP:Play()
