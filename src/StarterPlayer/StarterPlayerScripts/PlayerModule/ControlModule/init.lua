...
function ControlModule.new()
	...
	self.touchMovementController = nil
...
function ControlModule:OnRenderStepped(dt)
	if self.activeController and self.activeController.enabled and self.humanoid then
		...
		if self.humanoid:GetAttribute('CanJump') < tick() then
			local doit = self.activeController:GetIsJumping() or (self.touchJumpController and self.touchJumpController:GetIsJumping())
			self.humanoid.Jump = doit
			if doit then
				local conn; conn = self.humanoid.StateChanged:Connect(function(_, b)
					if b == Enum.HumanoidStateType.Landed then
						conn:Disconnect()
						self.humanoid:SetAttribute('CanJump', tick() + 1.5)
					end
				end)
			end
		end
...
function ControlModule:OnCharacterAdded(char)
	...
	self.humanoid:SetAttribute('CanJump', tick())
...
function ControlModule:SwitchToController(controlModule)
	...
	if self.activeController ~= self.controllers[controlModule] then
		...
		or self.activeControlModule == DynamicThumbstick) then
			...
			-- TODO: Complete touch movement module, and implement
			--self.
		else
