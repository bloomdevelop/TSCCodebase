...
local Darkness = game.SoundService.SoundStorage.Enviromental.Darkness
		...
		if (root.Position - script.Parent.Position).Magnitude <= MAD+5 then
			...
			if not Lighting.LightsOff.Outage.Value then
				...
				if LightsOn then
					...
					Darkness:Stop()
					game.SoundService.SoundStorage.Machinery.LightsOn:Play()
					Lighting.LightsOff.Enabled = false
					wait(3)
				else
					game.SoundService.SoundStorage.Machinery.LightsOn:Stop()
